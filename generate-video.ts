...
async function generateVideo(options: GenerateOptions): Promise<void> {
  ...
  fsExtra.ensureDirSync(framesDestDir);
  ...
  // Write props to a JSON file (avoids Windows escaping issues)
  const propsFilePath = path.join(remotionPublic, `${flowName}-props.json`);
  fs.writeFileSync(propsFilePath, JSON.stringify(remotionProps, null, 2));
  log.info(`Props written to: ${propsFilePath}`);
